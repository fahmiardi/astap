	...
	if(isset($_POST['cmd_submit']) && isset($_POST['uname']) && isset($_POST['cpass'])) { 	
		...
		if($_GET['act']=="login") {
			...
			if ($allowed === $sent) {
				...
				if($_POST['enctoken'] != $_SESSION['ASTA_TOKEN']) { 
					...
					if(authPelanggan($clean_user, $clean_password)) { 
						...
						$_SESSION['ASTA_USERNAME']=(string)$clean_user;
						session_id();
						if(!empty($_SESSION)) {
							session_regenerate_id(true);
						}																		
						$_SESSION['ASTA_PASSWORD']=(string)$clean_password;							
						...
						unset($_POST['enctoken']);
						unset($_SESSION['ASTA_TOKEN']);
						header("Location: index1.php?wrong");
					} 
				} 
			}
		} 
		...
		$query_login='	SELECT username, password 
						...
						AND password = "'.$_SESSION['ASTA_PASSWORD'].'" 
						AND idHak=2';
		...
		$_SESSION['ASTA_USERNAME'] = $row['username'];
		$_SESSION['ASTA_PASSWORD'] = $row['password'];
		session_id();
		if(!empty($_SESSION)) {
			session_regenerate_id(true);
		}
